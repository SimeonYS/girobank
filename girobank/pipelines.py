...
class GgirobankPipeline:

    # Database setup
    conn = sqlite3.connect('girobank.db')
    c = conn.cursor()

    def open_spider(self, spider):
        self.c.execute("""CREATE TABLE IF NOT EXISTS `girobank`
                         (date text, title text, link text, content text)""")

    def process_item(self, item, spider):
        self.c.execute("""SELECT * FROM girobank WHERE title = ? AND date = ?""",
        ...
        # Insert values
        self.c.execute("INSERT INTO girobank (date, title, link, content)"
